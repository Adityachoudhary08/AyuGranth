...
export default function ActionPlanSection({
  ...
  return (
    <section className={cn("bg-white border border-[#161412]/15 rounded-2xl p-6 sm:p-8 shadow-xs space-y-6", className)}>
      <div className="flex items-center justify-between border-b border-[#161412]/10 pb-3">
        <div>
          ...
          <p className="text-xs text-[#161412]/60 mt-0.5">
            Prioritized strategic actions grounded in legal and regulatory findings
          </p>
        </div>
      ...
      {actions.length === 0 ? (
        <p className="text-xs text-[#161412]/60 italic py-4 text-center">
          No recommended actions available for this analysis.
        </p>
      ) : (
        <div className="space-y-4">
          {actions.map((act) => (
            <div
              key={act.id}
              className="bg-[#FAF8F3] border border-[#161412]/15 rounded-xl p-5 flex flex-col md:flex-row md:items-start justify-between gap-4 hover:border-[#176B45]/40 transition-all"
            >
              ...
              {act.evidenceCount > 0 && (
                <div className="shrink-0 self-end md:self-center">
                  <span className="inline-flex items-center gap-1.5 text-[11px] font-mono font-semibold text-[#176B45] bg-white border border-[#161412]/10 px-3 py-1.5 rounded-lg shadow-2xs">
                    ...
                    <span>{act.evidenceCount} Sources Cited</span>
                  </span>
                </div>
              )}
            </div>
