...
export default function WhyThisResultModal({
  ...
  return <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50 backdrop-blur-xs animate-in fade-in duration-200">
      <div className="relative w-full max-w-xl bg-[#FAF8F3] border border-[#161412]/20 rounded-2xl shadow-2xl overflow-hidden" onClick={e => e.stopPropagation()}>
        {/* Header */}
        <div className="bg-[#176B45] text-white px-6 py-4 flex items-center justify-between border-b border-[#8C6D46]">
          <div className="flex items-center gap-2">
            ...
            <h3 className="font-serif text-base sm:text-lg font-medium">{t("whythisresultmodal.whyThisResult", "Why This Result?")}</h3>
          </div>
          <button type="button" onClick={onClose} className="p-1 rounded-lg text-white/80 hover:text-white hover:bg-white/10 transition-colors cursor-pointer" aria-label={t("whythisresultmodal.close", "Close")}>
            <X className="w-5 h-5" />
          </button>
        </div>
        ...
        <div className="p-6 sm:p-7 space-y-4 text-xs font-sans">
          <p className="text-[#161412]/70 font-serif italic text-sm mb-4">{t("whythisresultmodal.aayuGranthdecisionsupportchainconnecting", "AayuGranth decision-support chain connecting product characteristics to indexed legal sources:")}</p>

          {/* Step 1: Product Signal */}
          <div className="bg-white border border-[#161412]/15 rounded-xl p-4 shadow-xs">
            <div className="flex items-center justify-between mb-1">
              <span className="text-[10px] font-mono font-bold uppercase tracking-wider text-[#8C6D46]">{t("whythisresultmodal.1ProductSignal", "1. Product Signal")}</span>
          ...
          <div className="bg-white border border-[#161412]/15 rounded-xl p-4 shadow-xs">
            <div className="flex items-center justify-between mb-1">
              <span className="text-[10px] font-mono font-bold uppercase tracking-wider text-[#8C6D46]">{t("whythisresultmodal.3StatutoryInterpretation", "3. Statutory Interpretation")}</span>
          ...
          <div className="bg-[#176B45]/10 border border-[#176B45]/30 rounded-xl p-4 shadow-xs">
            <div className="flex items-center justify-between mb-1">
              <span className="text-[10px] font-mono font-bold uppercase tracking-wider text-[#176B45]">{t("whythisresultmodal.4ComputedFindingPosture", "4. Computed Finding / Posture")}</span>
        ...
        <div className="bg-[#FAF0DE] border-t border-[#d8cbb7] px-6 py-3.5 flex items-center justify-between text-[11px] font-mono text-[#6b5e4d]">
          <span>{t("whythisresultmodal.evidenceLinkedExplainability", "Evidence-Linked Explainability")}</span>
          <button type="button" onClick={onClose} className="px-3 py-1 bg-white border border-[#161412]/20 rounded-md text-xs font-sans font-semibold text-[#161412] hover:bg-[#FAF8F3] transition-colors cursor-pointer">{t("whythisresultmodal.close", "Close")}</button>
        </div>
      </div>
    </div>;
}