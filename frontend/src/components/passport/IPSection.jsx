...
export default function IPSection({
  ...
  return (
    <section id="section-ip" className={cn("bg-white border border-[#161412]/15 rounded-2xl p-6 sm:p-8 shadow-xs space-y-6", className)}>
      {/* Header */}
      <div className="flex items-center justify-between border-b border-[#161412]/10 pb-3">
        ...
        <span className="text-[10px] font-mono text-[#8C6D46] uppercase tracking-wider font-semibold">{t("productPassport.ui.section03", "Section 03")}</span>
      </div>
      ...
      <div className="bg-[#FAF6EC] border border-[#d8cbb7] rounded-xl p-5">
        <div className="flex flex-wrap items-center justify-between gap-2 mb-2">
          ...
          <span className="px-2.5 py-0.5 rounded font-mono text-[11px] font-bold bg-amber-100 text-amber-900 border border-amber-300">
            {t("ipsection.confidence", "Confidence:")} {translatePassportStatus(confidence, t)}
          </span>
        </div>

        <h4 className="font-serif text-base sm:text-lg font-bold text-[#161412] leading-snug mb-2">
          {translatePassportData(posture, t)}
        </h4>

        <p className="text-xs sm:text-sm text-[#161412]/80 leading-relaxed">
          {translatePassportData(reasoning, t)}
        </p>
      </div>
      ...
      <div>
        ...
        <div className="space-y-2">
          {risks.map((risk, idx) => (
            <div key={idx} className="flex items-start gap-2.5 p-3 rounded-xl bg-amber-50/50 border border-amber-200/60 text-xs">
              <ShieldAlert className="w-4 h-4 text-amber-700 shrink-0 mt-0.5" />
              <span className="text-[#161412]/90 leading-relaxed font-medium">{translatePassportData(risk, t)}</span>
            </div>
          ))}
        </div>
      </div>
      ...
      <div className="p-4 rounded-xl bg-[#FAF8F3] border border-[#161412]/10 text-xs">
        ...
        <p className="text-[#161412]/80 leading-relaxed">
          {translatePassportData(trademarkConsiderations, t)}
        </p>
      </div>
