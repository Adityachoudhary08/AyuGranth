...
export default function EvidenceBlock({
  ...
  return (
    <div className={cn(
      "bg-white border border-[#161412]/15 rounded-xl p-4 sm:p-5 shadow-xs transition-all hover:border-[#176B45]/40",
      className
    )}>
      {/* Top Meta Row */}
      ...
      {/* Excerpt if present */}
      {excerpt && (
        <div className="mt-2.5 p-2.5 bg-[#FAF8F3] border border-[#161412]/10 rounded-lg text-xs text-[#161412]/80 leading-relaxed font-serif italic">
          &ldquo;{excerpt}&rdquo;
        </div>
      )}
      ...
      <div className="mt-3.5 pt-3 border-t border-[#161412]/10 flex items-center justify-between gap-2 text-xs">
        <span className="text-[10px] font-mono text-[#161412]/50">
          Status: {status}
        </span>

        {whyThisResult && onOpenWhy && (
          <button
            type="button"
            onClick={() => onOpenWhy(whyThisResult)}
            className="inline-flex items-center gap-1 text-[11px] font-semibold text-[#176B45] hover:text-[#125537] transition-colors cursor-pointer"
          >
