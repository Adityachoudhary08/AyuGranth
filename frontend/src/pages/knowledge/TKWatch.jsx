    ...
    <div className="max-w-4xl mx-auto space-y-8">
      <div className="text-center mb-8">
        <h1 className="text-3xl font-serif text-[#176B45] mb-2 flex items-center justify-center gap-3">
          ...
          TK Misappropriation Watch
        </h1>
        <p className="text-[#161412]/60">Scan global patent filings for potential misappropriation of Indian traditional knowledge.</p>
      </div>
      ...
      <form onSubmit={handleSubmit} className="flex gap-4 max-w-2xl mx-auto">
        <div className="relative flex-1">
          ...
          <input
            ...
            placeholder="Enter Patent Publication Number or text snippet"
            className="w-full bg-white border border-[#161412]/20 rounded-full pl-12 pr-6 py-4 text-sm focus:outline-none focus:border-[#176B45] shadow-sm transition-all"
            required
          />
        </div>
        <button
          type="submit"
          disabled={isLoading || !patentNumber.trim()}
          className="px-6 py-4 bg-[#176B45] text-white font-medium rounded-full hover:bg-[#125537] transition-colors disabled:opacity-50"
        >
      ...
      {isLoading && (
        <div className="py-12 flex flex-col items-center justify-center">
          ...
          <p className="text-[#161412]/60 font-medium">Analyzing claims against TKDL...</p>
        </div>
      )}
      ...
      {result && (
        <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-500 max-w-3xl mx-auto">
          <div className={`p-6 border rounded-xl flex items-start gap-4 ${
            result.misappropriation_risk === 'High' ? 'bg-red-50 border-red-200 text-red-900' : 'bg-green-50 border-green-200 text-green-900'
          }`}>
            <AlertTriangle className={`w-6 h-6 shrink-0 mt-0.5 ${result.misappropriation_risk === 'High' ? 'text-red-600' : 'text-green-600'}`} />
            <div>
              <h2 className="text-lg font-bold mb-1">
                Risk Level: {result.misappropriation_risk}
              </h2>
          ...
          {result.evidence && result.evidence.length > 0 && (
            <div className="bg-white border border-[#161412]/10 rounded-xl overflow-hidden shadow-sm">
              <div className="px-5 py-3 border-b border-[#161412]/10 bg-[#f8f7f4]">
                <h3 className="font-serif text-lg text-[#161412]">Evidence of Prior Art</h3>
              </div>
              <ul className="divide-y divide-[#161412]/10">
                {result.evidence.map((ev, idx) => (
                  <li key={idx} className="p-5 space-y-2">
                    <p className="text-sm font-medium text-[#161412] bg-[#f8f7f4] p-3 rounded border border-[#161412]/10">
                      "Claim: {ev.claim}"
                    </p>
                    <div className="flex justify-center py-1">
                      <ArrowRight className="w-4 h-4 text-[#161412]/30" />
                    </div>
                    <p className="text-sm text-[#161412]/80 bg-[#176B45]/5 p-3 rounded border border-[#176B45]/20">
                      "Prior Art: {ev.prior_art}"
                    </p>
                  </li>
                ))}
              </ul>
