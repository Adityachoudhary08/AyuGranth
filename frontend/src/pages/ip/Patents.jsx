  ...
  return (
    <div className="max-w-5xl mx-auto space-y-8">
      <div>
        ...
        <p className="text-[#161412]/60">Evaluate formulations for potential patentability and Section 3(p) risks.</p>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        <div className="lg:col-span-1 space-y-6">
          <form onSubmit={handleSubmit} className="bg-white border border-[#161412]/10 rounded-xl p-5 shadow-sm space-y-4">
            <div>
              <label className="block text-sm font-medium text-[#161412] mb-1">Formulation Description</label>
              <textarea
                value={description}
                onChange={(e) => setDescription(e.target.value)}
                placeholder="Describe your formulation, ingredients, and processing methods..."
                rows={6}
            ...
            <button
              ...
              className="w-full py-2 px-4 bg-[#176B45] text-white rounded-md text-sm font-medium hover:bg-[#125537] transition-colors disabled:opacity-50 flex justify-center items-center gap-2"
            >
              {isLoading ? (
        ...
        <div className="lg:col-span-2 space-y-6">
          {!result && !isLoading && (
             <div className="h-full flex flex-col items-center justify-center text-center p-12 border border-[#161412]/10 border-dashed rounded-xl opacity-50 bg-white">
               ...
               <p className="text-[#161412] font-medium">Enter a formulation description</p>
               <p className="text-sm text-[#161412]/60 mt-1 max-w-sm">We'll assess novelty, inventive step, and traditional knowledge overlap.</p>
             </div>
          )}
          ...
          {isLoading && (
             <div className="h-full flex flex-col items-center justify-center text-center p-12 border border-[#161412]/10 rounded-xl bg-white shadow-sm">
               ...
               <p className="text-[#161412] font-medium">Analyzing Patentability...</p>
             </div>
          )}
          ...
          {result && (
            <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-500">
              ...
              {result.suggestions && result.suggestions.length > 0 && (
                <div className="bg-white border border-[#161412]/10 rounded-xl overflow-hidden shadow-sm">
                  <div className="px-5 py-3 border-b border-[#161412]/10 bg-[#f8f7f4]">
                    <h3 className="font-serif text-lg text-[#161412]">Strategic Suggestions</h3>
                  </div>
                  <ul className="p-5 space-y-3">
                    {result.suggestions.map((suggestion, idx) => (
                      <li key={idx} className="flex gap-3 text-sm text-[#161412]/80">
              ...
              {result.regime_map && (
                <div className="bg-white border border-[#161412]/10 rounded-xl overflow-hidden shadow-sm">
                  <div className="px-5 py-3 border-b border-[#161412]/10 bg-[#f8f7f4]">
                    <h3 className="font-serif text-lg text-[#161412]">IP Regime Applicability Map</h3>
                  </div>
                  <div className="divide-y divide-[#161412]/10">
                    {Object.entries(result.regime_map).map(([regime, entry]) => (
                      <div key={regime} className="p-5 flex items-start justify-between gap-4">
                        <div>
                        ...
                        <span className={cn(
                          ...
                          entry.applicable ? "bg-green-100 text-green-800" : "bg-gray-100 text-gray-600"
                        )}>
