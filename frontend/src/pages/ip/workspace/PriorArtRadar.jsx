...
export default function PriorArtRadar() {
  ...
  return (
    <div className="min-h-screen bg-[#faf8f3] font-sans pb-20">
      ...
      <main className="max-w-5xl mx-auto px-4 sm:px-6" style={{ paddingTop: '140px' }}>
        ...
        <div className="mb-8">
          ...
          <div className="flex items-center gap-3 mb-3">
            ...
            <h1 className="text-3xl font-serif text-[#161412]">Prior-Art Radar</h1>
          </div>
          <p className="text-[#161412]/60 max-w-2xl">
            Semantic similarity search across the legal and patent database to identify existing formulations.
          </p>
        </div>
        ...
        {/* Input Form */}
        {!result && !loading && (
          <div className="bg-white p-6 md:p-8 rounded-2xl border border-[#161412]/15 shadow-sm">
            <form onSubmit={handleSubmit} className="space-y-6">
              <div>
                <label className="block text-sm font-medium text-[#161412] mb-1.5">Formulation Description *</label>
                <p className="text-xs text-[#161412]/50 mb-2">Describe the formulation, ingredients, and method.</p>
                <textarea 
                  required 
                  name="formulation_description" 
                  value={formData.formulation_description} 
                  onChange={handleChange} 
                  className="w-full bg-[#f8f7f4] text-[#161412] border border-[#161412]/15 rounded-lg px-4 py-3 text-sm focus:outline-none focus:border-blue-500 transition-all resize-none min-h-[120px]" 
                  placeholder="e.g. A formulation containing Haritaki and Bibhitaki for digestive health..."
                />
              </div>

              <div className="flex items-center gap-4">
                <div className="w-1/3">
                  <label className="block text-sm font-medium text-[#161412] mb-1.5">Max Results</label>
                  <select 
                    name="top_k" 
                    value={formData.top_k} 
                    onChange={handleChange} 
                    className="w-full bg-[#f8f7f4] text-[#161412] border border-[#161412]/15 rounded-lg px-4 py-3 text-sm focus:outline-none focus:border-blue-500 transition-all"
                  >
                ...
                <div className="w-2/3 pt-6">
                  <button type="submit" className="w-full py-3.5 bg-blue-600 text-white font-medium rounded-xl hover:bg-blue-700 transition-all shadow-sm flex items-center justify-center gap-2">
                    <Search className="w-4 h-4" /> Scan Prior-Art
                  </button>
                </div>
              </div>
            </form>
        ...
        {/* Loading State */}
        {loading && (
          <div className="bg-white border border-[#161412]/10 rounded-2xl p-16 flex flex-col items-center justify-center shadow-sm animate-pulse min-h-[400px]">
            <div className="w-12 h-12 rounded-full border-4 border-blue-100 border-t-blue-600 animate-spin mb-6"></div>
            <h3 className="text-xl font-serif text-[#161412]">Scanning Database...</h3>
            <p className="text-sm text-[#161412]/50 mt-2 text-center max-w-sm">
              Performing vector similarity search against registered patents and legal chunks.
            </p>
          </div>
        )}
          ...
          <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4">
            ...
            <div className="bg-white rounded-2xl border border-[#161412]/15 shadow-sm p-6 md:p-8 flex flex-col md:flex-row items-center justify-between gap-8">
              <div className="w-full md:w-1/2 flex items-center justify-between md:justify-start gap-8">
                <div>
                  <h2 className="text-[10px] font-bold uppercase tracking-widest text-[#161412]/50 mb-1">Overall Relevance</h2>
                ...
                <div>
                  <h2 className="text-[10px] font-bold uppercase tracking-widest text-[#161412]/50 mb-1">Max Similarity</h2>
              ...
              <div className="w-full md:w-1/2">
                ...
                <div className="w-full h-3 bg-[#f8f7f4] rounded-full overflow-hidden border border-[#161412]/10">
                  <div 
                    className={cn("h-full transition-all duration-1000 ease-out", getRelevanceBarColor(result.overall_relevance))} 
                    style={{ width: `${Math.min(Math.max(result.max_similarity * 100, 0), 100)}%` }}
                  ></div>
                </div>
                <div className="mt-4 flex justify-end">
                  <button onClick={reset} className="text-xs font-medium px-4 py-2 bg-[#f8f7f4] border border-[#161412]/10 text-[#161412] rounded-lg hover:bg-white transition-colors">
                    New Search
                  </button>
                </div>
              </div>
            ...
            <div className="bg-white rounded-2xl border border-[#161412]/15 shadow-sm overflow-hidden">
              <div className="px-6 py-4 border-b border-[#161412]/10 bg-[#faf8f3]/50 flex items-center justify-between">
                <h3 className="text-sm font-serif text-[#161412] flex items-center gap-2">
                  <Database className="w-4 h-4 text-blue-600" /> Discovered Prior-Art Matches ({result.results.length})
                </h3>
              </div>
              
              {result.results.length === 0 ? (
                <div className="p-8 text-center">
                  <p className="text-sm text-[#161412]/60">No significant prior-art matches found in the database.</p>
                </div>
              ) : (
                <div className="divide-y divide-[#161412]/10">
                  {result.results.map((item, idx) => (
                    <div key={idx} className="p-6 group hover:bg-[#faf8f3]/30 transition-colors">
                      <div className="flex flex-col md:flex-row justify-between items-start gap-4 mb-3">
                        ...
                        <div className="flex items-center gap-3 bg-[#f8f7f4] px-3 py-1.5 rounded-lg border border-[#161412]/5">
                          ...
                          <div className="flex flex-col items-end">
                            <span className="text-[10px] font-bold text-[#161412]/40 uppercase tracking-wider leading-none mb-1">Similarity</span>
