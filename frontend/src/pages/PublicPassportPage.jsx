...
export default function PublicPassportPage() {
  ...
  if (error) {
    return (
      <main className="min-h-screen bg-[#FAF8F3] text-[#161412] flex items-center justify-center px-5">
  ...
  if (!summary) {
    return (
      <main className="min-h-screen bg-[#FAF8F3] text-[#161412] flex items-center justify-center px-5">
        <p className="text-sm text-[#161412]/60">Verifying passport...</p>
      </main>
    );
  }

  return (
    <main className="min-h-screen bg-[#FAF8F3] text-[#161412] px-5 py-10 sm:py-16">
      <div className="max-w-3xl mx-auto space-y-8">
        <header className="border-b border-[#161412]/10 pb-6 flex items-start justify-between gap-5">
          <div className="flex items-start gap-3">
            ...
            <div>
              <p className="font-mono text-xs font-bold uppercase tracking-[0.2em] text-[#176B45]">AayuGranth</p>
              <p className="text-[11px] text-[#161412]/55 mt-1">Public passport verification</p>
            </div>
          </div>
          <div className="inline-flex items-center gap-1.5 rounded-full border border-[#176B45]/25 bg-[#176B45]/5 px-3 py-1.5 text-[10px] font-mono font-bold uppercase tracking-wider text-[#176B45]">
            <ShieldCheck className="w-3.5 h-3.5" /> Verified by AayuGranth
          </div>
        </header>

        <section className="bg-white border border-[#161412]/10 rounded-2xl p-6 sm:p-8 shadow-sm">
          <p className="text-[10px] font-mono font-bold uppercase tracking-[0.2em] text-[#8C6D46]">Authenticity summary</p>
          ...
          <dl className="grid grid-cols-1 sm:grid-cols-3 gap-5 mt-8 pt-5 border-t border-[#161412]/10">
            <div>
              <dt className="text-[10px] font-mono uppercase tracking-wider text-[#161412]/50">Passport ID</dt>
              <dd className="font-mono font-bold text-sm mt-1">{summary.passport_id}</dd>
            </div>
            <div>
              <dt className="text-[10px] font-mono uppercase tracking-wider text-[#161412]/50">Generated</dt>
              <dd className="text-sm mt-1">{formatDate(summary.generated_at)}</dd>
            </div>
            <div>
              <dt className="text-[10px] font-mono uppercase tracking-wider text-[#161412]/50">Last analyzed</dt>
        ...
        <section>
          <div className="flex items-center justify-between gap-4 mb-3">
            <h2 className="font-serif text-2xl">Domain status</h2>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
            {summary.domain_statuses.map((domain) => (
              <div key={domain.label} className={`flex items-center justify-between gap-3 rounded-xl border px-4 py-3 ${statusStyles[domain.color] || statusStyles.yellow}`}>
        ...
        <aside className="rounded-xl border border-[#8C6D46]/25 bg-[#FAF6EC] px-4 py-4 text-sm text-[#161412]/75">
          <strong className="text-[#161412]">Public authenticity summary.</strong> {summary.privacy_note}
        </aside>
      </div>
    </main>
  );
