...
export default function ProductPassportPage() {
  ...
  const handleDeletePassport = async () => {
    ...
    const confirmed = window.confirm(`Delete "${productName}" permanently? This passport will be permanently deleted from the records. This action cannot be undone.`);
    if (!confirmed) return;
    try {
  ...
  const DEFAULT_INITIAL_DATA = {
    ...
    biodiversity: {
      ...
      obligations: ['Obtain prior approval from National Biodiversity Authority (NBA) via Form I for commercial utilization', 'Mandatory disclosure of biological resource geographical origin in patent filings (Section 6)', 'Ensure Prior Informed Consent (PIC) and fair and equitable benefit-sharing compliance'],
      ...
      classicalReferences: ['Charaka Samhita — Chikitsa Sthana (Rasayana Vidhi)', 'Ayurvedic Pharmacopoeia of India (API) Monograph Index', 'TKDL-related public research papers and case studies (not direct database access)'],
      evidence: {
  ...
  // ─────────────────────────────────────────────────────────────
  if (loading) {
    return <div className="min-h-screen bg-[#FAF8F3] font-sans text-[#161412] pb-24">
        <Navbar />
        <main className="max-w-5xl mx-auto pt-[140px] px-4 sm:px-6 space-y-8 animate-pulse">
          {/* Header Skeleton */}
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
            {[1, 2, 3, 4].map(i => <div key={i} className="h-36 bg-white border border-[#161412]/10 rounded-xl p-4 space-y-3">
  ...
  if (error && !passportData) {
    return <div className="min-h-screen bg-[#FAF8F3] font-sans text-[#161412] pb-24">
        ...
        <main className="max-w-md mx-auto pt-40 px-4 text-center space-y-4">
          ...
          <h2 className="text-2xl font-serif font-bold text-[#161412]">{t("productpassportpage.unabletoLoadProduct", "Unable to Load Product Passport")}</h2>
          <p className="text-xs text-[#161412]/70 leading-relaxed">{t("productpassportpage.wecouldntretrievethe", "We couldn't retrieve the requested product passport data from the intelligence index right now.")}</p>
          <div className="pt-2 flex justify-center gap-3">
            <button type="button" onClick={fetchPassport} className="inline-flex items-center gap-2 px-4 py-2 bg-[#176B45] text-white rounded-lg text-xs font-bold uppercase tracking-wider hover:bg-[#125537] transition-colors cursor-pointer">
              <RefreshCw className="w-3.5 h-3.5" />
              <span>{t("productpassportpage.retry", "Retry")}</span>
            </button>
            <Link to="/patent-passport" className="inline-flex items-center gap-2 px-4 py-2 bg-white border border-[#161412]/20 text-xs font-semibold text-[#161412] rounded-lg hover:bg-[#FAF8F3] transition-colors">
              <span>{t("productpassportpage.intakeForm", "Intake Form")}</span>
            </Link>
          </div>
        </main>
      </div>;
  ...
  return <div className={cn("min-h-screen bg-[#FAF8F3] font-sans text-[#161412] pb-28", printMode === 'card' && "print-card-mode")}>
      ...
      <Navbar />
      {error && <div className="max-w-5xl mx-auto px-4 sm:px-6 pt-24 print:hidden">
          <div className="rounded-xl border border-amber-300 bg-amber-50 px-4 py-3 text-xs text-amber-900">{error}</div>
        </div>}
      {analysisLoading && <div className="max-w-5xl mx-auto px-4 sm:px-6 pt-24 print:hidden">
          <div className="rounded-xl border border-[#176B45]/20 bg-[#176B45]/5 px-4 py-3 text-xs text-[#176B45]">{t("productpassportpage.productloadedIndexingregulatory", "Product loaded. Indexing regulatory, IP, biodiversity, and export evidence in the background...")}</div>
        </div>}

      <main className="max-w-5xl mx-auto pt-[140px] px-4 sm:px-6 space-y-10">
        
        {/* Header with Breadcrumb, Title, and 2 Download Actions */}
        <PassportHeader passportId={passportData.passportId} productName={p.name} onDownloadCard={handleDownloadCard} onDownloadFullDossier={handleDownloadFullDossier} onExportFullData={handleExportFullData} onDelete={handleDeletePassport} />

        {/* HERO: VIRTUAL DIGITAL IDENTIFICATION CARD */}
        <div id="passport-card-target" className="pt-2">
          <ProductPassportCard productName={p.name} productCategory={p.category} passportId={passportData.passportId} productId={passportId} originJurisdiction={p.countryOfOrigin || 'India'} targetMarket={p.targetCountry || 'United States'} status={passportData.status} lastAnalyzed={passportData.lastAnalyzed} evidenceCount={passportData.evidenceCount} onDownloadCard={handleDownloadCard} />
        </div>
        ...
        <div className={cn("space-y-10 passport-dossier-body", printMode === 'card' && "print:hidden")}>
          ...
          <IntelligenceOverview regulatoryStatus={reg.status} regulatorySummary={reg.summary} ipStatus={ip.status} ipSummary={ip.summary} tkStatus={bio.tkOverlapStatus} tkSummary={bio.tkSummary} internationalStatus={intl.status} internationalSummary={intl.summary} />

          {/* SECTION 1: PRODUCT PROFILE */}
          <ProductProfileSection product={p} />

          {/* SECTION 2: REGULATORY INTELLIGENCE */}
          <RegulatorySection classification={reg.classification} pathway={reg.pathway} requirements={reg.requirements} claimsConsideration={reg.claimsConsideration} openIssues={reg.openIssues} jurisdiction={reg.jurisdiction} evidence={reg.evidence} onOpenWhy={setWhyModalData} />

          {/* SECTION 3: IP & NOVELTY INTELLIGENCE */}
          <IPSection posture={ip.posture} confidence={ip.confidence} reasoning={ip.reasoning} risks={ip.risks} trademarkConsiderations={ip.trademarkConsiderations} evidence={ip.evidence} onOpenWhy={setWhyModalData} />

          {/* SECTION 4: BIODIVERSITY & TRADITIONAL KNOWLEDGE */}
          <BiodiversitySection isBiological={bio.isBiological} absApplicable={bio.absApplicable} absSummary={bio.absSummary} absObligations={bio.obligations} tkOverlapStatus={bio.tkOverlapStatus} tkSummary={bio.tkSummary} classicalReferences={bio.classicalReferences} evidence={bio.evidence} onOpenWhy={setWhyModalData} />

          {/* SECTION 5: INTERNATIONAL MARKET READINESS */}
          <InternationalSection origin={intl.origin} targetCountry={intl.targetCountry} status={intl.status} summary={intl.summary} requirements={intl.requirements} missingInfo={intl.missingInfo} evidence={intl.evidence} onOpenWhy={setWhyModalData} />

          {/* RECOMMENDED ACTIONS PLAN */}
          <ActionPlanSection actions={passportData.actions} />

          {/* ANALYSIS HISTORY & TIMELINE */}
          <PassportTimeline history={passportData.history} lastAnalyzed={passportData.lastAnalyzed} />

          {/* VERIFICATION & QR BLOCK */}
          <PassportVerification passportId={passportData.passportId} productId={passportId} generatedDate={passportData.generatedAt} lastUpdated={passportData.lastUpdated} evidenceCount={passportData.evidenceCount} status={passportData.status} />
      ...
      <WhyThisResultModal isOpen={Boolean(whyModalData)} onClose={() => setWhyModalData(null)} data={whyModalData} />
    </div>;
}